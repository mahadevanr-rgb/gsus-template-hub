...
/**
 * KanbanItemView
 * Standalone presentational component for a Kanban card item.
 */
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mt-3 pt-2">
        ...
        {/* Keyboard accessible movement buttons */}
        {!isOverlay && (
...
/**
 * SortableKanbanItem
 * Wraps item in @dnd-kit/sortable for drag and keyboard control.
 */
...
/**
 * KanbanColumn
 * Droppable column container hosting sortable items.
 */
...
/**
 * KanbanCard
 * Full-featured Kanban board card with REAL drag-and-drop state transitions, DragOverlay,
 * keyboard accessibility, and multi-column support.
 */
export default function KanbanCard({
  ...
  const handleDragOver = (event) => {
    ...
    if (!targetColumn) {
      targetColumn = findColumnOfItem(overId);
    }
    ...
    const newColumns = columns.map((col) => {
      if (col.id === sourceColumn.id) {
        return {
          ...col,
          items: col.items.filter((i) => i.id !== activeItemId),
        };
      }
      if (col.id === targetColumn.id) {
        // Insert either before the hovered item or at the end
  ...
  // Keyboard / button movement helper
  ...
  return (
    <Card className={`w-full max-w-4xl p-5 sm:p-7 rounded-3xl shadow-xl overflow-hidden ${className}`}>
      <DndContext
        sensors={sensors}
